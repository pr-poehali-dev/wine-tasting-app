...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if method == "GET" and action == "list":
            friend_rows = qall(conn,
                f"SELECT u.id,u.nickname,u.bio,u.avatar FROM {SCHEMA}.users u "
                f"JOIN {SCHEMA}.friendships f ON f.friend_id=u.id WHERE f.user_id=%s ORDER BY f.created_at DESC",
                (user_id,))
            friends = []
            for fid, nickname, bio, avatar in friend_rows:
                ...
                for tr in t_rows:
                    ...
                    tastings.append({
                        "id": str(tr[0]), "name": tr[1], "year": tr[2] or "", "country": tr[3] or "",
                        "region": tr[4] or "", "style": tr[5], "date": str(tr[6]) if tr[6] else "",
                        "rating": tr[7], "photo": tr[8] or "", "color": tr[9] or "",
                        "density": tr[10] or "", "aromaIntensity": tr[11] or 0,
                        "primaryAromas": tr[12] or "", "impression": tr[13] or "",
                        "likes": lk[0] if lk else 0, "producer": "", "secondaryAromas": "",
                        "flavor": "", "finish": "", "notes": "", "is_liked": False
                    })
                friends.append({
                    "id": str(fid), "nickname": nickname, "bio": bio or "", "avatar": avatar or "",
                    "tastingsCount": cnt[0] if cnt else 0,
                    "avgRating": round(float(cnt[1]), 1) if cnt and cnt[1] else 0,
                    "tastings": tastings
                })
        ...
        if method == "GET" and action == "search":
            ...
            rows = qall(conn,
                f"SELECT id,nickname,bio,avatar FROM {SCHEMA}.users WHERE nickname ILIKE %s AND id!=%s LIMIT 10",
                (f"%{nick}%", user_id))
            results = []
            for fid, n, bio, avatar in rows:
                ...
                results.append({
                    "id": str(fid), "nickname": n, "bio": bio or "", "avatar": avatar or "",
                    "tastingsCount": cnt[0] if cnt else 0, "is_friend": isf is not None
                })
            return {"statusCode": 200, "headers": CORS, "body": json.dumps(results)}

        if method == "POST" and action == "add":
            b = json.loads(event.get("body") or "{}")
            fid = int(b.get("friend_id", 0))
            if not fid:
        ...
        if method == "POST" and action == "remove":
            b = json.loads(event.get("body") or "{}")
            fid = int(b.get("friend_id", 0))
            qrun(conn, f"DELETE FROM {SCHEMA}.friendships WHERE user_id=%s AND friend_id=%s", (user_id, fid))
            conn.commit()
            return {"statusCode": 200, "headers": CORS, "body": json.dumps({"ok": True})}

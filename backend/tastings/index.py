...
def tasting_row_to_dict(row, likes_count=0, is_liked=False):
    return {
        ...
        "color": row[11],
        "density": row[12],
        "aromaIntensity": row[13],
        "primaryAromas": row[14],
        "secondaryAromas": row[15],
        "flavor": row[16],
        "finish": row[17],
        "rating": row[18],
        "notes": row[19],
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        # GET / — список своих или чужих дегустаций
        if method == "GET" and (path.endswith("/tastings") or path.endswith("/tastings/")):
        ...
        # POST / — создать дегустацию
        if method == "POST" and (path.endswith("/tastings") or path.endswith("/tastings/")):
            if not user_id:
                return {"statusCode": 401, "headers": CORS, "body": json.dumps({"error": "Не авторизован"})}
        ...
        if method == "POST" and "/like/" in path:
            ...
            tasting_id = int(path.split("/like/")[-1].rstrip("/"))
